...
/*        //递归出口
    ...
    /**
     * 两路快排  处理小于pivot的 和 大于pivot的
     *      从前向后走，遇到大于pivot的停下   从后向前走，遇到小于pivot的停下   交换前判断要交换的下标合法性
     *      然后交换了继续向中间走
     * @param array
     * @param low
     * @param high
     ...
     * @return
     */
    public static void partition3(int[] array,int low,int high) {
        if (low >= high) {
            return;
        }
        //随机选取一个下标
        ...
        while (i < highJ) {
            if (array[i] < pivot) {  //小于
                ...
                //后移
                i++;
            } else if (array[i] > pivot) {  //大于
                swap(array,i,highJ-1);
                highJ--;
            } else {    //等于
                i++;
            }
        }
        //将分区点位置交换到<pivot的区间最后,然后当前lowI所属位置已经属于== pivot了
        swap(array,low,lowI);
        partition3(array,low,lowI-1);
        partition3(array,highJ,high);
    }
